...
gerrit_ssh.run = function(command, config) {
  ...
  logger.debug("gerrit_ssh.run: " + command);
...
gerrit_ssh.query = function(query, config) {
  ...
  return gerrit_ssh.run(queryString, config).then(function(result) {
...
gerrit_ssh.query.number = function(number, config) {
  return gerrit_ssh.query(["change:%s project:%s limit:1", number, config.project], config);
};
...
// TODO replace with https://www.npmjs.com/package/spawn-please
function promisifiedSpawn(command, args) {
  var bufStdOut = [];
  var bufStdErr = [];
  return new Q(function(resolve, reject) {
    var child = spawn.spawn(command, args);
