...
git.isIndexClean = function() {
  return git.execSuccess("diff --no-ext-diff --quiet --exit-code");
};
...
git.branch.create = function(name, checkout) {
  if (checkout) {
    return git("checkout -b \"%s\"", name);
  }
  else {
    return git("branch \"%s\"", name);
  }
};

git.branch.upstream = function(upstream) {
  if (upstream) {
    // set
    return git("branch --set-upstream-to=\"%s\"", upstream);
  }
  else {
    // get
    return git("rev-parse --symbolic-full-name --abbrev-ref '@{u}'");
  }
};
