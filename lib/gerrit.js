...
gerrit.changeid_for_topic = function(topic, project, config) {
  return gerrit_ssh.query(["project:%s topic:%s limit:1", project, topic], config).then(function(result) {
...
gerrit.clone = function(gerrit_name, project_name, destination_folder) {
  ...
  if (fs.exists(destination_folder)) {
    return Q.reject(util.format("Destination %s already exists", destination_folder));
  }
...
gerrit.status = function() {
  ...
  return gerrit_ssh.query(["status:open project:%s", project], gerrit.repoConfig());
};
...
gerrit.checkout = function(target, patch_set, force_branch_overwrite) {
  ...
  if (change_id) {
    ...
    q = gerrit.changeid_for_topic(topic, gerrit.repoProject(), config).then(function(result) {
...
gerrit.review = function(verified_score, code_review_score, message, action) {
  ...
  command.push(util.format("--project '%s'", project));
