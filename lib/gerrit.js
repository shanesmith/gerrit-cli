...
gerrit.installHook = function(remote) {
  ...
  return mkdirp(hooks_dir).then(function() {
    ...
    return gerrit_ssh.scp("hooks/commit-msg", hooks_dir, gerrit.remoteConfig(remote));
...
gerrit.ssh = function(command, remote) {
  ...
  return gerrit_ssh(command, gerrit.remoteConfig(remote));
};

gerrit.status = function(remote) {
  requireInRepo();

  var config = gerrit.remoteConfig(remote);
...
gerrit.assign = function(reviewersArray, remote) {
  ...
  reviewersArray.forEach(function(reviewer) {
    ...
    gerrit_ssh(["set-reviewers --add '%s' -- %s", reviewer, hash], gerrit.remoteConfig(remote)).then(function() {
