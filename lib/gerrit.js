...
gerrit.addRemote = function(remote_name, gerrit_name, project_name, install_hook) {
  return requireInRepo()
    ...
    .then(function(config) {
      ...
      git.exec("remote add '%s' '%s'", remote_name, source_url);
  ...
  return requireInRepo()
    ...
    .then(function(result) {
      ...
      if (!patch_set) {
        ...
        var remote_refs = git.exec("ls-remote '%s' '%s/*'", remote, ref).split("\n");
  ...
  return git.branch.list()
    .filter(function(branch) {
      ...
      return revlist.every(function(commit) {
        ...
        return !!git.exec("rev-list --grep='%s' --before='%s' -n1 %s", changeId, beforeDate, upstream);
      });
