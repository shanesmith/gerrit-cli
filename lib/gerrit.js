    ...
    .then(function(config) {
      ...
      if (change_id) {
        ...
        return gerrit_ssh.query.change_id(change_id, config)
          .then(function(result) {
            if (result.length === 0) {
              throw new GerritError(["Could not find change id \"%s\" in project \"%s\"", change_id, config.project]);
            }
            return result[0];
          });
        ...
        return gerrit_ssh.query.topic(target, config)
          .then(function(result) {
            if (result.length === 0) {
              throw new GerritError(["Could not find topic \"%s\" in project \"%s\"", target, config.project]);
            }
            return result[0];
          });
