...
gerrit.remoteConfig = function(remote) {
...
gerrit.installHook = function(remote) {
  ...
  return requireInRepo()
    ...
    .then(function() {
      return gerrit.remoteConfig(remote);
    })
    .then(function(config) {
      return gerrit_ssh.scp("hooks/commit-msg", hooks_dir, config);
    });
...
gerrit.assign = function(reviewersArray, remote) {
  ...
  return requireInRepo()
    ...
    .then(function(config) {
      ...
      currentReviewers = git.config("gerrit.reviewers", {all: true}) || [];

      return _.map(reviewersArray, function(reviewer) {
        return gerrit_ssh(["set-reviewers --add '%s' -- %s", reviewer, hash], config).return(reviewer);
      });
...
gerrit.push = function(base_branch, is_draft, remote) {
  ...
  return requireInRepo()
    ...
    .then(function(config) {
      ...
      return git.show(["push", "origin", "HEAD:"+ref]);
    })
    .then(function() {
      if (should_undraft) {
  ...
  return requireInRepo()
    ...
    .then(function(config) {
      ...
      if (change_id) {
        ...
        return gerrit_ssh.query.change_id(change_id, config)
          .then(function(result) {
            if (result.length === 0) {
              throw new GerritError(["Could not find change id \"%s\"", change_id]);
            }
            return result[0];
          });
        ...
        return gerrit_ssh.query.topic(target, config)
          .then(function(result) {
            if (result.length === 0) {
              throw new GerritError(["Could not find topic \"%s\".", target]);
            }
            return result[0];
          });
    ...
    .then(function(result) {
      ...
      if (!patch_set) {
        ...
        var remote_refs = git("ls-remote origin '%s/*'", ref).split("\n");
...
gerrit.review = function(verified_score, code_review_score, message, action, remote) {
