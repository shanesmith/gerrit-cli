...
gerrit.up = function(base_branch, is_draft, remote) {
  ...
  return requireInRepo()
    .then(function() {
      ...
      base_branch = base_branch || git.config.get(util.format("branch.%s.merge", topic));

      base_branch = git("rev-parse --symbolic-full-name --abbrev-ref '%s'", base_branch);
    ...
    .then(function() {
      remote = remote || git.config.get(util.format("branch.%s.remote", topic));
      return gerrit.parseRemote(remote);
    })
    .then(function(remoteConfig) {
  ...
  return requireInRepo()
    ...
    .then(function(removeBranch) {
      ...
      if (topic && !git.branch.exists(topic)) {
        ...
        git.branch.setUpstream(remote + "/" + branch);
      }
