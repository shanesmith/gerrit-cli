...
cli.clone = function(config_name, project_name, destination_folder, options) {
  ...
  if (!config_name) {
    ...
    q = q
      .then(function() {
        var projects = _.keys(gerrit.allConfigs());
...
cli.assign = function(reviewersArray, options) {
  requireInRepo();
  gerrit.assign(reviewersArray, options.remote);
};
...
cli.review = function(verified_score, code_review_score, message, options) {
  requireInRepo();
  gerrit.review(verified_score, code_review_score, message, options.remote);
};
...
cli.comment = function(message, options) {
  requireInRepo();
  gerrit.review(null, null, message, options.remote);
};
