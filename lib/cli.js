      ...
      .then(function(config) {
        ...
        if (config && !options.edit) {
          ...
          return prompter.prompt([{
              ...
              type: "input",
              name: "url",
              message: "URL " + chalk.grey("(ex: http://example.com/gerrit/)"),
...
cli.projects = function(options) {
  ...
  gerrit.projects(config).then(function(projects) {
...
cli.clone = function(project_name, destination_folder, options) {
  ...
  q.then(function() {
    ...
    gerrit.clone(config, project_name, destination_folder);
...
cli.patches = function(options) {
  ...
  gerrit.patches(query, options.remote).then(function(patches) {
...
cli.up = function(options) {
  ...
  if (options.assign.length && revList.length > 1) {
  ...
  .then(function() {
    options.all = true;
    cli.assign(options.assign, options);
  });
...
cli.pubmit = function(options) {
  requireInRepo();
  requireUpstream();
  _push(options.remote, options.branch, false)
    .then(function() {
...
cli.web = function(options) {
  gerrit.config("default")
  .then(function(config) {
    var hash = git.hashFor("HEAD");
    var url = util.format("%s/#/q/%s,n,z", config.url, hash);
